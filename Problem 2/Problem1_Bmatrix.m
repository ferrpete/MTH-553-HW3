...
function B = Problem1_Bmatrix(n_interior,h,dt,theta,km,kp)
...
values(1:n_interior-1) = km(2:end)*(1-theta)*(dt/(h^2));
values(n_interior:2*n_interior-1) = 1 - km*(1-theta)*(dt/(h^2)) - kp*(1-theta)*(dt/(h^2));
values(2*n_interior:3*n_interior-2) = kp(1:end-1)*(1-theta)*(dt/(h^2));
